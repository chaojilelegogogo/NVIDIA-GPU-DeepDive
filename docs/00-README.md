...
> 编号约定：Part 编号全局唯一且**严格连续**（part01~part20，卷一 = 01~03，卷二 = 04~13，卷三~卷七 = 14~20）。`src/` 下的实验目录按同一编号组织（如 `src/part10-sass/` 对应 Part 10）。
...
全书还有三条隐藏的主线，会在 Part 3（硬件发展史）汇合成一张全景图：
...
**[`roadmap-ampere-hopper-blackwell.md`](vol1-gpu-hardware/roadmap-ampere-hopper-blackwell.md) —— 按 Ampere → Hopper → Blackwell 架构代际组织的实战路线图**，把 Part 2/5/6/7 里分散在各主题下的知识点，重新串成三条循序渐进的学习轨道（Track 1 Ampere：Tensor Core/`cp.async`/Async Pipeline；Track 2 Hopper：Warp Group/WGMMA/TMA/Distributed Shared Memory；Track 3 Blackwell：TMEM/第五代 Tensor Core/新 SM Pipeline/新 PTX-MMA 指令），每个 Track 结尾都明确写出"遗留了什么问题，逼着下一代必须继续改"，并标注了每个知识点在消费级显卡（如 RTX 50 系列）上是否可以实操验证。

## 知识体系结构（不是按 API 分类，是按因果链分类）

```
GPU 硬件发展 (Part 3, 全书灵魂)
      │  为什么会长成这样
      ▼
GPU 执行模型 (Part 1)
      │  硬件长什么样、SIMT 怎么跑
...
CUDA 编程模型 (Part 4)
      │  软件如何映射到硬件
      ▼
CUDA Memory (Part 5) ── 全书最重要，占比最大
      │  数据怎么在层次化存储间流动
      ▼
执行流水线 (Part 2)
      │  指令怎么发射、怎么隐藏延迟
      ▼
Tensor Core 发展史 (Part 6) ── 全书核心
      │  算力单元怎么从 FMA 进化到 tcgen05
      ▼
同步手册 (Part 13) ── 以硬件同步原语为轴
      │  执行会合 / 内存排序 / 异步完成
      ▼
PTX (Part 7) / CUDA C++ API (Part 8) / 编译器 (Part 9) / SASS (Part 10)
...
### 第二部分 · CUDA 执行流水线 —— [`part02-execution-pipeline.md`](vol1-gpu-hardware/part02-execution-pipeline.md)
**核心问题：指令怎么被调度、怎么隐藏延迟？**
...
### 第三部分 · GPU 硬件发展史（全书灵魂）—— [`part03-gpu-hardware-history.md`](vol1-gpu-hardware/part03-gpu-hardware-history.md)
...
### 第四部分 · CUDA 编程模型 —— [`part04-cuda-programming-model.md`](vol2-cuda-software/part04-cuda-programming-model.md)
**核心问题：软件怎么映射到硬件？**
Grid/Block/Thread/Warp 的编程语义、**Hopper Thread Block Cluster 执行层级**（`clusterDim`/`block_rank`/共同调度；DSM 细节指向 Part 5）、Kernel Launch、Stream/Event、Occupancy、Memory Model、同步原语层级（含 `cluster.sync`）。结尾回答：为什么 Block 不能跨 SM？为什么还要 Cluster？为什么 Shared Memory 属于 Block？

### 第五部分 · CUDA Memory（全书最重要）—— [`part05-cuda-memory/00-overview.md`](vol2-cuda-software/part05-cuda-memory/00-overview.md)
...
### 第六部分 · Tensor Core 指令全景与编程手册（全书核心）—— [`part06-tensor-core-handbook/00-overview.md`](vol2-cuda-software/part06-tensor-core-handbook/00-overview.md)
...
### 第七部分 · CUDA 特殊指令 / PTX —— [`part07-ptx-instructions.md`](vol2-cuda-software/part07-ptx-instructions.md)
**核心问题：PTX 指令对应什么硬件？**
...
### 第八部分 · CUDA C++ API —— [`part08-cuda-cpp-api.md`](vol2-cuda-software/part08-cuda-cpp-api.md)
...
### 第九部分 · CUDA 编译器 —— [`part09-cuda-compiler.md`](vol2-cuda-software/part09-cuda-compiler.md)
...
### 第十部分 · SASS（高级）—— [`part10-sass.md`](vol2-cuda-software/part10-sass.md)
**核心问题：怎么读机器码、分析 stall？**
Instruction Format、谓词化、控制码（Stall Count/Yield Flag/Barrier/Reuse Flag）、Register Allocation、Instruction Scheduling、为什么同样 CUDA 代码会生成不同 SASS、用 Nsight Compute 做 Warp State/Stall 归因分析。配套最小探针：[`src/part10-sass/`](../src/part10-sass/README.md)（`__fmaf_rn` → `fma.rn.f32` → `FFMA`，Blackwell/Thor 可直接跑）。
...
**核心问题：框架代码如何被编译成 Part 6/7 里那些 kernel？**
...
配套 CUDA 实验统一放在仓库根目录 `src/`，按文档 Part 和章节编号组织。当前可跑套件包括：[Part 10 SASS / FFMA 探针](../src/part10-sass/README.md)、Part 13 四维同步：[总览](../src/part13-synchronization-handbook/README.md)、[13.1 Execution Barrier](../src/part13-synchronization-handbook/01-execution-barriers/README.md)、[13.2 Memory Ordering](../src/part13-synchronization-handbook/02-memory-ordering/README.md)、[13.3 Async Pipeline](../src/part13-synchronization-handbook/03-async-pipelines/README.md)、[13.4 Tensor](../src/part13-synchronization-handbook/04-tensor-synchronization/README.md)、[13.5 Collectives/Atomics](../src/part13-synchronization-handbook/05-collectives-and-atomics/README.md)。每个 case 可生成 executable、PTX、CUBIN 与 SASS。

1. **不要跳过 Part 1**：不涉及一行 CUDA 代码，但决定了你后面看到每个 API 时是否能"看到硬件"。
2. **Part 3 建议反复读**：第一遍在读完 Part 1、Part 4 后读一次（建立时间线），学完 Part 5~10 后再读一次（这时每个名词背后都有具体技术细节支撑）。
...
5. 建议的进度节奏（对应开头 4~6 个月的估计）：
   - 第 1~2 周：Part 1、Part 4（硬件基础 + 编程模型）
   - 第 3~6 周：Part 5、Part 2（Memory + 流水线，全书篇幅最大的部分）
   - 第 7~9 周：Part 6（Tensor Core 演进史 + 指令编程手册，建议配合 CUTLASS 示例反复回顾）
   - 第 10~12 周：Part 7~10（PTX / C++ API / 编译器 / SASS，软件栈四层）
   - 第 13~14 周：Part 3（硬件发展史通读第二遍）
