...
## 8.2 CUDA Runtime API vs CUDA Driver API
...
- **Driver API**（`cuMemAlloc`、`cuLaunchKernel`、`cuModuleLoad`、`cuTensorMapEncodeTiled`……）：更底层、更细粒度地控制 Context、Module（对应编译好的 cubin/PTX）、显式的 Kernel 参数打包与启动。第五部分讲的 TMA 描述符构建（`cuTensorMapEncodeTiled`）就必须通过 Driver API，因为它是 Runtime API 尚未完全封装的底层能力。
...
Stream 已在第四部分讲过语义。这里补充 **CUDA Graph**：
...
Cooperative Groups 回答的是：**CUDA API 如何表达硬件协作能力**。执行层级见[第四部分 2.2.4](part04-cuda-programming-model.md)；DSM 见[第五部分 3.10](part05-cuda-memory/08-cluster-dsm-tmem.md)；barrier 语义见[第十三部分](part13-synchronization-handbook/01-execution-barriers.md)。
...
### 8.4.2 `thread_block_tile` 与 warp 级协作
...
| `map_shared_rank(ptr, rank)` | 把本地 shared 指针映射到 peer CTA | **DSM 寻址** → Part 05 |
...
### 8.4.6 `memcpy_async`（Group 封装）

`cooperative_groups::memcpy_async` 是对第五部分 `cp.async` / pipeline 路径的 C++ 封装，以 Group 为单位发起集体异步拷贝。完成仍属 async completion，不等于 `block.sync()` / `cluster.sync()`。

## 8.5 `cuda::pipeline` 与 `cuda::barrier`（`<cuda/pipeline>`, `<cuda/barrier>`）

这是对第二部分讲的"生产者-消费者流水线"和第七部分讲的 `mbarrier` 的标准 C++ 封装（来自 libcu++），核心类型：
...
- **`cuda::pipeline<cuda::thread_scope_block>`**：封装"发起异步拷贝（`producer_commit`）→ 等待完成（`consumer_wait`）→ 释放缓冲区（`consumer_release`）"的完整协议，并支持指定流水线级数（stage 数），直接对应第二部分的 Double/Triple Buffer 概念——用 `cuda::pipeline` 写多级流水线的 GEMM，不需要手写底层的 `cp.async.commit_group`/`wait_group` 序列。
...
对应第六部分 Volta 引入的 Warp 级 Tensor Core 操作：
...
WMMA、`mma.sync`、WGMMA、数据中心 `tcgen05` 与消费级 `sm_120` block-scale 路径的边界，见[第六部分 Tensor Core 指令手册](part06-tensor-core-handbook/00-overview.md)。

## 8.7 从 API 到 PTX 到 SASS：一个可以亲自验证的完整例子
...
对一个使用 `cuda::memcpy_async` 的简单 kernel，满足架构、对齐和 API 前提时，PTX **可能**出现 `cp.async.cg.shared.global` 及配套 group 指令，Ampere SASS 常见 `LDGSTS`。对 WMMA，PTX 可出现 `wmma.*`，也可能被编译器降低为其它相关 MMA 形式；目标 SASS 常见 `HMMA` 系列。应将这些视为指定 Toolkit/target 下的实测结果，而非稳定的一一映射；完整验证流程见第六部分 5.7 节。
