# 6.4 Part 5：Turing/Ampere —— `mma.sync`、`ldmatrix` 与 `cp.async`
...
## 6.4.3 `ldmatrix`：Shared Memory → 正确的寄存器 fragment
...
## 6.4.4 `cp.async`：为什么 Tensor Core 章节要学习它
