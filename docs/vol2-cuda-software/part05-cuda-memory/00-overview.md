# 第五部分 CUDA Memory：从存储层次到 Blackwell TMA
...
| [08-cluster-dsm-tmem.md](08-cluster-dsm-tmem.md) | DSM 地址/性能、`map_shared_rank`、TMA multicast、TMEM 如何组成完整数据通路？（Cluster 执行模型见 Part 04） |
| [09-debug-performance.md](09-debug-performance.md) | 如何判断正确性、检查生成指令、定位没有重叠或 bank conflict？ |

## 节号与文件位置映射

> 历史说明：本部分原为单文件「第三部分」，节号为 3.x；分卷重构后改为「第五部分」，节号相应变为 5.x（如原 §3.8 `cp.async` 即现 §5.8）。下表给出节号到文件的映射；新增专题以子节形式插入，不重新解释旧编号：

| 节号 | 主题 | 文件位置 |
|---|---|---|
| 5.1 | 存储空间总览 | [01-memory-hierarchy.md](01-memory-hierarchy.md) |
| 5.2–5.5 | Coalescing、Alignment、Bank Conflict、Transaction | [02-access-patterns.md](02-access-patterns.md) |
| 5.6–5.7 | Tiled GEMM 动机、`ld.global + st.shared` | [03-data-movement-evolution.md](03-data-movement-evolution.md) |
| 5.8 | Ampere `cp.async` | [04-ampere-cp-async.md](04-ampere-cp-async.md) |
| 5.9 | Hopper bulk copy / TMA | [05-hopper-bulk-and-tma.md](05-hopper-bulk-and-tma.md) |
| 5.9 扩展 | Blackwell TMA 指令与 TensorMap 模式 | [06-blackwell-tma.md](06-blackwell-tma.md) |
| 5.8–5.9 实战 | cp.async/TMA pipeline | [07-pipeline-examples.md](07-pipeline-examples.md) |
| 5.10–5.11 | Cluster/DSM、TMEM | [08-cluster-dsm-tmem.md](08-cluster-dsm-tmem.md) |
| 5.12–5.13 | 跨代总结与诊断 | [本页](00-overview.md)、[09-debug-performance.md](09-debug-performance.md) |

相关深入阅读：[第四部分 Cluster 执行层级](../part04-cuda-programming-model.md)、[第八部分 `cluster_group` API](../part08-cuda-cpp-api.md)、[第二部分执行流水线](../../vol1-gpu-hardware/part02-execution-pipeline.md)、[第六部分 Tensor Core 手册](../part06-tensor-core-handbook/00-overview.md)、[第十一部分性能优化](../part11-performance-optimization.md)、[第十三部分 Cluster barrier / 异步同步](../part13-synchronization-handbook/01-execution-barriers.md)。
