...
### 10.1.1 最小实操：CUDA `__fmaf_rn` → PTX `fma` → SASS `FFMA`

配套 case：[`src/part10-sass/case01_ffma.cu`](../../src/part10-sass/case01_ffma.cu)。目标是在 Blackwell（本仓库实测为 Thor / `sm_110`）上走通一遍“源码 → PTX → SASS”，不强求性能。
...
# PTX / SASS：
#   build/src/part10-sass/artifacts/part10_case01_ffma.ptx
#   build/src/part10-sass/artifacts/part10_case01_ffma.sass
```
...
## 10.2 Instruction Format：一条 SASS 指令长什么样
...
### 10.2.1 谓词化（Predication）：另一种处理分支的手段
...
## 10.3 控制码：Stall / Yield / Barrier / Reuse

延续第二部分的铺垫，这里给出更完整的控制码字段构成（现代架构上，通常打包为一个约 23 位的控制字，逻辑上对应）：
...
## 10.4 Dependency 与 Scoreboard（承接第二部分，补充细节）

第二部分讲了 Scoreboard 的基本机制，这里补充两个实践中重要的细节：
...
## 10.5 Register Allocation：为什么同样的 CUDA 代码会生成不同的 SASS
...
## 10.6 Instruction Scheduling：`ptxas` 如何排布指令顺序
...
2. 均衡不同执行单元（ALU、LD/ST、SFU、Tensor Core）的指令分布，充分利用 Dual Issue（第二部分 4.5 节）的机会。
...
## 10.7 用 Nsight Compute 看 SASS、分析 Stall
...
- **Warp State Statistics**：把 Stall 归因到具体原因（常见的几类）：
  - `stall_long_scoreboard`：等待一次长延迟操作（典型是 Global Memory 访问）的 Scoreboard——如果这个占比很高，说明访存延迟没有被充分隐藏，需要检查 Occupancy 是否足够、或访存模式是否高效（回到第五部分的 Coalescing 诊断）。
...
## 10.8 小结：SASS 分析的核心方法论

读 SASS/用 Nsight Compute 分析性能，本质上是把前面第一到第九部分讲的所有硬件知识，串联起来做"侦探式"的证据链推理：

```
观察到的现象（如 stall_long_scoreboard 占比高）
      ↓ 对照第二部分：Scoreboard 机制
定位到具体是哪条/哪类指令在等（如 LDG）
      ↓ 对照第五部分：这次访存是否 Coalesced？是否该用 cp.async/TMA 重叠？
定位到根因（如访存模式差 / Occupancy 不足以掩盖延迟）
      ↓ 对照第四部分：调整 Block 大小 / 寄存器用量以提升 Occupancy
      ↓ 对照第五部分：改善访存模式 / 引入异步流水线
验证：重新编译、重新用 Nsight Compute 测量，确认 stall 占比下降、总耗时降低
```

这套"现象 → 硬件机制 → 根因 → 具体优化手段 → 验证"的闭环，正是第十一部分要系统化整理的性能优化实战方法论。而在此之前，我们先在第三部分把前面所有的硬件演进线索，按时间顺序完整串一遍——因为只有理解了每一代硬件"新增了什么、为什么增加、软件怎么支持、性能提升在哪"，第十一部分的优化手段才有明确的"针对哪一代硬件、利用哪一个新特性"的具体抓手。
