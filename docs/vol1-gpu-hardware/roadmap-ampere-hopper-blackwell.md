...
在正式进入 Ampere 之前，确保你已经清楚以下"前 Ampere 基线"（对应 Volta/Turing，详见第六部分 5.3/5.4），因为 Ampere 的每一个新特性都是相对这个基线做的改进：

- Tensor Core 从 Volta（2017）就存在了，最早的编程接口是 Warp 级的 `wmma`/`mma.sync`（原子形状 `m8n8k4`），只支持 FP16 输入（第六部分 5.3）。
- 数据从 Global Memory 搬到 Shared Memory，在 Ampere 之前只有一种方式：`ld.global` 读入寄存器，再 `st.shared` 写出——**必须经过发起线程的寄存器**（第五部分 3.7）。
- 没有软件可显式控制的"流水线"抽象，双缓冲需要程序员完全手写同步逻辑。

如果这三点还不熟悉，建议先回读第六部分 5.3~5.4 和第五部分 3.7。
...
**深度内容跳转**：第六部分 5.4 节（TF32/BF16 的设计动机、`mma.sync` 具体形状、`ldmatrix` 为什么出现）；PTX 语法见第七部分 6.4 节表格。
...
**深度内容跳转**：第五部分 3.7（问题铺垫）→ 3.8（Why/What/How/Evolution 完整展开）；C++ API 见第八部分 7.5；完整的 group commit/wait 语义见[第十三部分 13.3](../vol2-cuda-software/part13-synchronization-handbook/03-async-pipelines.md)。
...
**深度内容跳转**：第二部分 4.8（Producer-Consumer 模式）、4.9（Double/Triple Buffer/Ping-Pong）、4.11（`cuda::pipeline` 软件接口）；完整代码骨架见第八部分 7.5。
...
按第五部分 3.8 节 Evolution 小节的原话："`cp.async` 虽然去掉了数据路过寄存器这一步，但仍然要求**每个参与的线程各自计算自己那一份数据的源/目标地址、各自发起一条指令**"。具体拆解成三个遗留问题：
...
**深度内容跳转**：第六部分 5.5 节；第二部分 4.8 节 Producer-Consumer 在 Warp Group 层面的硬件化。
...
**深度内容跳转**：第六部分 5.5 节；PTX 语法与 `mma.sync` 对照见第七部分 6.4 表格；WGMMA fence/commit/wait 依赖协议见[第十三部分 13.4](../vol2-cuda-software/part13-synchronization-handbook/04-tensor-synchronization.md)。
...
**深度内容跳转**：第五部分 3.9（完整 Why/What/How/Evolution）；PTX 指令见第七部分 6.2.2（`mbarrier`）与 6.3（`cp.async.bulk.tensor.*`）；phase/transaction completion 状态机见[第十三部分 13.3](../vol2-cuda-software/part13-synchronization-handbook/03-async-pipelines.md)。
...
**深度内容跳转**：第五部分 3.10（完整 Why/What/How/Evolution，新增章节）；第四部分 2.2.1 节回顾"Block 不能跨 SM"这条基本规则为什么在 DSM 之下依然成立。
...
按第五部分 3.9 节 Evolution 小节：TMA 和 WGMMA 已经把"搬运"和"更大协作颗粒度"这两个问题解决得很好，但两个新问题浮现：
...
**深度内容跳转**：第五部分 3.11（完整 Why/What/How/Evolution）；第六部分 5.6 节。
...
**深度内容跳转**：第六部分 5.6 节（消费级/数据中心分叉与 block-scale MMA）。
...
**深度内容跳转**：第二部分 4.10（新增章节，完整展开这条新流水线的三个变化：存储通路、发起颗粒度、跨 SM 协作范围）；第五部分 3.10/3.12（DSM 与 CTA Pair 的关系）；第六部分 5.6。
...
**深度内容跳转**：第七部分 6.4 节表格（已标注数据中心/消费级差异）；[第十三部分 13.4](../vol2-cuda-software/part13-synchronization-handbook/04-tensor-synchronization.md) 给出 `tcgen05` commit/fence/wait 与 TMEM 生命周期的同步边界；官方 PTX ISA 手册中 `tcgen05` 章节（随 CUDA Toolkit 版本更新，务必核对你使用的 CUDA 版本对应的手册）。

**动手验证**：在数据中心 Blackwell（或云实例）上，写一个完整的 `tcgen05.alloc → tcgen05.mma → tcgen05.ld → tcgen05.dealloc` 序列，反汇编确认 SASS 中出现的 TMEM 相关操作码（第六部分调研资料中提到的 opcode 范围，如 122-139 号段）；在消费级 RTX 50 上，对照验证同样的 GEMM 用 `mma.sync.aligned.block_scale` 实现，比较两条路径在各自硬件上的吞吐表现。
...
|---|---|---|---|
| Ampere | Tensor Core（TF32/BF16/`mma.sync`/`ldmatrix`） | Part 6 §6.3 | 是（RTX 30/40/50 均可） |
| Ampere | `cp.async` | Part 5 §5.7~5.8 | 是（`sm_80`+ 均可） |
| Ampere | Async Pipeline | Part 2 §2.8~2.9、§2.11；Part 8 §8.5 | 是 |
| Hopper | Warp Group / Warp Specialization | Part 6 §6.4.1、§6.4.3；Part 2 §2.8 | 否（需要 H100/H200 或云实例） |
| Hopper | WGMMA | Part 6 §6.4.2 | 否 |
| Hopper | TMA | Part 5 §5.9 | 否 |
| Hopper | Distributed Shared Memory / Cluster | Part 5 §5.10 | 否 |
| Blackwell | TMEM | Part 5 §5.11 | **否**（仅 B100/B200/B300） |
| Blackwell | 第五代 Tensor Core（FP4/FP6/NVFP4） | Part 6 §6.5.2~6.5.3、§6.5.6 | **部分可以**（RTX 50 用 `mma.sync.aligned.block_scale` 路径） |
| Blackwell | 新 SM Pipeline（`tcgen05` 流水线/CTA Pair） | Part 2 §2.10；Part 6 §6.5.5 | **否**（仅数据中心） |
| Blackwell | 新 PTX/MMA 指令（`tcgen05` 族） | Part 7 §7.4 | **否**（RTX 50 对应学 `mma.sync.aligned.block_scale`） |
